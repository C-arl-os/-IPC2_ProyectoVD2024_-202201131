...
def cargarUsuarios():
    ...
    try:
        ...
        for usuario in root:
            ...
            # Validar si el id empieza con 'IPC'
            ...
            # Validar si el correo termina con '.com' y el teléfono tiene 8 caracteres o menos
            if not (correo.endswith('.com') and len(telefono) <= 8):
                continue

            # Validar si el id ya está repetido
    ...
    for usuario in lista_usuarios:
        ...
        for imagen in lista_imagenes:
            if imagen.id_usuario == usuario.id:
                ET.SubElement(imagenes, 'imagen', id=str(imagen.id), id_usuario=str(imagen.id_usuario), editado=str(imagen.editado))
